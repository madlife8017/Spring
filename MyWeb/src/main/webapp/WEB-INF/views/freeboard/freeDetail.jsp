    ...
    window.onload = function () {
        ...
        */
        ...
        document.getElementById('replyList').addEventListener('click', e => {
            ...
            console.log('댓내용 ' + content);

            //3. 모달 창 하나를 이용해서 상황에 따라 수정 / 삭제 모달을 구분하기 위해서
            //조건문을 작성. (모달 하나로 수정, 삭제를 같이 처리. 그러기 위해 디자인 조정.)
            if(e.target.classList.contains('replyModify')) {
                //수정 버튼을 눌렀으므로 수정 모달 형식을 꾸며 주겠다.
        ...
        document.getElementById('modalDelBtn').onclick=()=>{
            ...
            //3서버에서 요청받아서 비번확이나고, 비번이 맞으면 삭제 진행
            //4. 만약 틀리면 문자열반황해서 '비번틀렷습니다'
            fetch('${pageContext.request.contextPath}/reply/'+rno,reqObj)
            .then(res => res.text())
            .then(data=>{
            ...
            //5. 삭제완료디ㅗ면 모달 닫고 목록 요청 다시 보내세요 (reset여부를 판단 ㄱ)
        } //end delete event

        //댓글 날짜 변환 함수
        function parseTime(regDateTime){
            let year,month,dat,hour,minute,second;

            if(regDateTime.length==5){
               [year,month,dat,hour,minute] = regDateTime;
                second = 0;

            } else{
                [year,month,dat,hour,minute,second] = regDateTime;
            }
            
            
            //원하는 날짜로 객체를 생성
            const regTime = new Date(year,month-1,dat,hour,minute,second);
            const date = new  Date();
            const gap = date.getTime()-regTime.getTime();

            let time;
            if(gap<60*60*24*10000){
                if(gap<60*60*1000){
                    time='방금전';
                } else{
