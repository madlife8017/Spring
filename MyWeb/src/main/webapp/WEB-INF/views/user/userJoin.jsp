...
<script>
	...
	document.getElementById('idCheckBtn').onclick = function(){
		...
		if(userId===''){
			...
			- Promise를 객체를 자동으로 리턴하여 손쉽게 통신의 응답데이터를
			소비할수 있게 해줍니다. (Promise: 비동기 통신의 순서를 보장하는 문법)
			...
			*/
			...
			//fetch('url',{요청관련 정보 객체})
			const result = fetch('${pageContext.request.contextPath}/user/idCheck',{
			...
			//Promise 객체의 상태가 요청성공일시 데이터 후속 처리 진행.
			.then(res=>{

				//fetch 함수를 통해 비동기 통신이 실행되고,
				//요청이 완료된 후 then()의 매개값으로 응답에 관련된
				// 함수를 콜백 방식으로 전달합니다.
				// 함수의 매개변수를 선언하면 해당 매개벼수로 응답에 관련된
			...
			*/
			...
			//비동기 요청 보내기
			fetch('${pageContext.request.contextPath}/user/idCheck',requObj)
			.then(res=>res.text())//요청완료후 응답정보에서 텍스트만 빼기
			.then(data => {

				if(data==='ok'){
					//더이상 아이드를 작성 할 수 없도록 막아주겟다.	
